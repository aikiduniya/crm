...
export default function Documents() {
  ...
  const handleDelete = async () => {
    ...
    try { const { error } = await supabase.from("documents").delete().eq("id", editItem.id); if (error) throw error; toast({ title: "Document deleted" }); setDeleteOpen(false); setEditItem(null); queryClient.invalidateQueries({ queryKey: ["documents"] }); }
    catch (err: any) { toast({ title: "Error", description: err.message, variant: "destructive" }); }
    setSaving(false);
  };
